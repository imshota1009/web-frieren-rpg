document.addEventListener('DOMContentLoaded', () => {
    ...
    let gameMap = [];
    let mapSize = 0;
    let temporaryMapChanges = {}; // For Flower Garden spell
    ...
    function updateSpellbookScreen() {
        ...
        player.spells.forEach(spellName => {
            ...
            const li = document.createElement('li');
            li.innerHTML = `
                <span>${spell.name} (MP: ${spell.cost})</span>
            `;
    ...
    function castUtilitySpell(spellName) {
        ...
        if (spell.effect === 'appraise_chest') {
            ...
            const directions = [
                { x: 0, y: -1, name: '北' }, { x: 0, y: 1, name: '南' }, 
                { x: -1, y: 0, name: '西' }, { x: 1, y: 0, name: '東' }
            ];
            ...
            for (const dir of directions) {
                ...
                if (targetY >= 0 && targetY < mapSize && targetX >= 0 && targetX < mapSize) {
                    ...
                    if (tileCode === 'B') {
                        ...
                        if (!chest || chest.opened) {
                            addLog(`${dir.name}にある宝箱は空のようだ。`, "system");
                        } else if (chest.content.type === 'mimic') {
                            if (Math.random() < 0.99) {
                                addLog(`【${dir.name}】強大な魔力を感じる…これはミミックだ！`, "system");
                            } else {
