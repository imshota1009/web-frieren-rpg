document.addEventListener('DOMContentLoaded', () => {
    ...
    const bossDatabase = {
        "aura": {
            ...
            stats: { hp: 1500, mp: 1000, atk: 40, def: 30, spd: 25, luck: 15 },
            exp: 5000,
            gold: 1000,
            isBoss: true,
            drops: [{ name: "魔石", chance: 1.0, quantity: 10 }],
            abilities: [
                { type: 'damage', name: '斬撃', power: 1.1 },
                { type: 'damage', name: '魔力の斬撃', power: 1.5, cost: 50 },
    ...
    const enemyDatabase = {
        ...
        boss: [] // No random encounters on boss maps
    };
    ...
    const showScreen = (screenId) => {
        ...
        // BGM Control based on screen
        if (screenId === 'main-game-screen') {
            playBgm('map-bgm');
        } else if (screenId === 'battle-screen') {
            // Battle BGM is handled in startBattle
            ...
            for (let x = 0; x < mapSize; x++) {
                ...
                if (tempChange && tempChange.type === 'flower_garden') {
                    ...
                    switch(tileCode) {
                        ...
                        case 'o': symbol = '💧'; break;
                        case 'S': case 'M': case 'D': case 'V': symbol = '🌀'; break;
                        case 'B':
    ...
    function updateBattleScreen() {
        ...
        runButton.disabled = currentEnemy.isBoss; // Can't run from bosses
