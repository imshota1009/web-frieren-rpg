document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadMap(mapId, targetX, targetY) {
        ...
        for (let y = 0; y < mapSize; y++) {
            ...
            for (let x = 0; x < mapSize; x++) {
                ...
                let type = 'floor';
                if (tileCode === 'f') type = 'forest';
                if (tileCode === 'p') type = 'plains';
                if (tileCode === 'T') type = 'town';
                if (tileCode === 'C') type = 'cave_entrance';
                if (tileCode === 'w') type = 'wall';
                if (tileCode === 'E') type = 'cave_exit';
                if (tileCode === 's') type = 'snow';
                if (tileCode === 'S') type = 'snow_portal';
                if (tileCode === 'M') type = 'forest_portal';
                gameMap[y][x] = { type };
    ...
    const playerAction = (action) => {
        ...
        if (action === 'run') {
            ...
            if (Math.random() < 0.25) { // 25% success chance
    ...
    function loseBattle() {
        ...
        if(townPortal) {
            ...
            // Failsafe in case no town is found
            loadMap(player.currentMap);
        }
        player.stats.hp = 1;
        updateHUD();
